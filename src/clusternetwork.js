...
export const _networkSubclusterSeparator = ".";
export const _networkGraphAttrbuteID = "patient_attribute_schema";
export const _networkNodeAttributeID = "patient_attributes";
export const _networkMissing = __("general")["missing"];
export const _networkReducedValue = "Different (other) value";
export const _networkMissingOpacity = "0.1";
export const _networkMissingColor = "#999";
export const _networkContinuousColorStops = 9;
...
export const _defaultFloatFormat = d3.format(",.2r");
...
var hivtrace_cluster_network_graph = function (
  ...
  self.uniqValues = helpers.get_unique_values(json.Nodes, new_schema);
  ...
  if (json.Settings && json.Settings.created) {
    ...
    self.today =
      options && options["today"] ? options["today"] : helpers.getCurrentDate();
  }
                  ...
                  action: function (button, v) {
                    if (clustersOfInterest.get_editor()) {
                      clustersOfInterest.get_editor().append_node_objects(
                        _.filter(cluster.children, (n) => nodeset.has(n.id) && n.priority_flag > 0)
                      );
                    }
                    return false;
                  },
      ...
      },
      {
        ...
        generator: function (cluster) {
          ...
          definition["actions"] = function (item, value) {
            ...
            if (clustersOfInterest.get_editor() && cluster.priority_score.length > 0) {
              result = result.concat(
                _.map(cluster.priority_score, (c) => {
                  const nodeset = new Set(c);
                  return {
  ...
  self.get_ui_element_selector_by_role = function (role, not_nested) {
    ...
    return (
      (not_nested ? "" : "#" + button_bar_ui) + helpers.get_ui_element_selector_by_role(role)
    );
  };
  ...
  helpers.dateTimeInit(options, self._is_CDC_, helpers._networkCDCDateField);

  if (self._is_CDC_) {
    self._additional_node_pop_fields.push(helpers._networkCDCDateField);
  }
  ...
  self.filter_time_period = function (cluster) {
    return _.some(self.nodes_by_cluster[cluster.cluster_id], (n) =>
      self.attribute_node_value_by_id(n, helpers.getClusterTimeScale()) >=
  ...
  if (self.cluster_attributes) {
    ...
    if (
      ...
      self.using_time_filter = helpers.getCurrentDate();
      self.warning_string += __("network_tab")["cluster_display_info"];
  ...
  self.cluster_display_filter = function (cluster) {
    return _.every(self.cluster_filtering_functions, (filter) => filter(cluster));
  };
      ...
      map: function (node) {
        try {
          var value = self._parse_dates(
            self.attribute_node_value_by_id(node, helpers._networkCDCDateField)
          );
  ...
  var handle_cluster_click = function (cluster, release) {
    ...
    if (cluster) {
      ...
      const show_on_map_enabled = _.every(cluster.children, (node) => self._get_node_country(node).length === 2);
    ...
    var event_handler = function (network, e) {
      ...
      if (!network.network_cluster_dynamics) {
        ...
        network.handle_inline_charts = function (plot_filter) {
          ...
          svgPlots.plot_cluster_dynamics(
  ...
  self._refresh_subcluster_view = function (set_date) {
    self.annotate_priority_clusters(helpers._networkCDCDateField, 36, 12, set_date);
          ...
          enclosure
            ...
            .attr(
              ...
              _defaultDateViewFormatSlider(
                d3.min(network.nodes, (node) => network.attribute_node_value_by_id(
                  node,
  ...
  var oldest_nodes_first = function (n1, n2) {
    const date_field = date_field || helpers._networkCDCDateField;
  ...
  self.annotate_priority_clusters = function (
    ...
    */
    ...
    try {
      ...
      var cutoff_long = helpers.getNMonthsAgo(start_date, span_months);
      var cutoff_short = helpers.getNMonthsAgo(start_date, recent_months);
        ...
        _.each(subclusters, (sub) => {
          ...
          var rr_cluster = _.filter(
            hivtrace_cluster_depthwise_traversal(
              ...
              _.filter(subcluster_json.Edges, (e) => e.length <= self.subcluster_threshold)
            ),
            (cc) => cc.length > 1
          );
  ...
  function default_layout(packed) {
    ...
    if (self.clusters.length === 0) {
      self.clusters = init_layout.filter((v, i, obj) => !(typeof v.cluster_id === "undefined"));
    } else {
  ...
  function change_window_size(delta, trigger) {
    if (delta) {
      ...
      var rescale_x = d3.scale.linear().domain(
        d3.extent(network_layout.nodes(), (node) => node.x)
      );
      rescale_x.range(
        _.map(rescale_x.domain(), (v) => v * x_scale)
      );
      //.range ([50,self.width-50]),
      var rescale_y = d3.scale.linear().domain(
        d3.extent(network_layout.nodes(), (node) => node.y)
      );
      rescale_y.range(
        _.map(rescale_y.domain(), (v) => v * y_scale)
      );
    ...
    self.nodes.forEach((n) => {
      _.defer((a_node) => {
        ...
        if (neighborhood_size < 2) {
          a_node.lcc = helpers.HIVTRACE_UNDEFINED;
        } else if (neighborhood_size > 500) {
          a_node.lcc = helpers.HIVTRACE_TOO_LARGE;
        } else {
          // count triangles
  ...
  self.compute_local_clustering_coefficients_worker = _.once(() => {
    ...
    worker.onmessage = function (event) {
      ...
      nodes.forEach((n) => {
        ...
        node_to_update.lcc = n.lcc ? n.lcc : helpers.HIVTRACE_UNDEFINED;
      });
    };
  ...
  self.compute_global_clustering_coefficients = _.once(() => {
    ...
    self.clusters.forEach((c) => {
      _.defer((a_cluster) => {
        ...
        if (cluster_size < 3) {
          a_cluster.cc = helpers.HIVTRACE_UNDEFINED;
        } else if (estimate_cubic_compute_cost(a_cluster, true) >= 5000000) {
          a_cluster.cc = helpers.HIVTRACE_TOO_LARGE;
        } else {
  ...
  self.mark_nodes_as_processing = function (property) {
    self.nodes.forEach((n) => {
      n[property] = helpers.HIVTRACE_PROCESSING;
    });
  };
  ...
  function initial_json_load() {
    ...
    self.cluster_sizes = [];
    self.update_volatile_elements = (container) => {
      tables.update_volatile_elements(self, container);
    };
    ...
    self._extract_attributes_for_nodes = function (nodes, column_names) {
      var result = [
        _.map(column_names, (c) => c.raw_attribute_key),
      ];
    ...
    self._extract_exportable_attributes = function (extended) {
      ...
      return_array.push(
        _.filter(self.json[_networkGraphAttrbuteID], (d) => d.type in allowed_types)
      );
    ...
    self._extract_nodes_by_id = function (id) {
      return _.filter(self.nodes, (n) => n.cluster.toString() === id.toString() || n.subcluster_label === id.toString());
    };
    ...
    self._cluster_list_view_render = function (
      ...
      if (group_by_attribute) {
        _.each(column_ids, (column) => {
          var binned = _.groupBy(cluster_nodes, (n) => self.attribute_node_value_by_id(n, column.raw_attribute_key));
    ...
    if (button_bar_ui) {
      ...
      var cluster_commands = [
        [
          ...
          () => {
            const colorScheme = helpers.getCurrentCategoryColorMapping(
              self.uniqValues,
              self.colorizer
            );
    ...
    self.edges = graph_data.Edges.filter((v, i) => v.source in connected_links && v.target in connected_links);
    ...
    self._aux_populate_category_menus = function () {
      if (button_bar_ui) {
        ...
        var valid_shapes = _.filter(valid_cats, (d) => (
        ...
        const color_stops = _.get(graph_data, colorStopsPath, _networkContinuousColorStops);
        ...
        var valid_scales = _.filter(
          _.map(graph_data[_networkGraphAttrbuteID], (d, k) => {
            ...
            if (d.type === "Number" || d.type === "Number-categories") {
              var values = _.filter(
                _.map(graph_data.Nodes, (nd) => self.attribute_node_value_by_id(
                  nd,
                  k,
                  d.type === "Number"
                )),
        ...
        const _menu_label_gen = (d) => (d["annotation"] ? "[" + d["annotation"] + "] " : "") + d["label"];
  ...
  function update_network_string(node_count, edge_count) {
    if (network_status_string) {
      ...
      const clusters_selected = _.filter(self.clusters, (c) => (
        !c.is_hidden && c.match_filter !== undefined && c.match_filter > 0
      )).length;

      const nodes_selected = _.filter(self.nodes, (n) => n.match_filter && !n.is_hidden).length;
      ...
      //         + clusters_shown +"</strong> clusters are expanded. Of <strong>" + self.edges.length + "</strong> edges, <strong>" + self.draw_me.edges.length + "</strong>, and of  <strong>" + self.nodes.length  + " </strong> nodes,  <strong>" + self.draw_me.nodes.length + " </strong> are displayed. ";
      // if (singletons > 0) {
  ...
  function draw_a_node(container, node) {
    if (node) {
      ...
      container
        .selectAll("path")
        .attr("d", svgPlots.generate_svg_symbol(symbol_type).size(node_size(node)))
      ...
      container
        //.attr("d", svgPlots.generate_svg_symbol(symbol_type).size(node_size(node)))
        .attr("class", "node")
        .classed("selected_object", (d) => d.match_filter && !self.hide_unselected)
  ...
  function check_for_predefined_shapes(cat_id) {
    ...
    if (cat_id in _networkPresetShapeSchemes) {
      ...
      return {
        domain: domain,
        range: _.map(domain, (v) => _networkPresetShapeSchemes[
          cat_id
      ...
      _.each(
        ...
        (value, key) => {
          ...
          self.legend_svg
            ...
            .attr("d", svgPlots.generate_svg_symbol(shape_mapper(value)).size(128))
            .classed("legend", true)
            .style("fill", "none");
  ...
  self.filter = function (conditions, skip_update) {
    ...
    conditions = _.map(["re", "distance", "date"], (cnd) => _.map(
    ...
    if (conditions[1].length) {
      ...
      _.each(self.edges, (e) => {
        var did_match = _.some(conditions[1], (d) => d.greater_than ? e.length >= d.value : e.length < d.value);
    ...
    if (conditions[2].length) {
      self.nodes.forEach((n) => {
        ...
        n.date_filter = _.some(conditions[2], (d) => node_T >= d[0] && node_T <= d[1]);
      });
    } else {
      self.nodes.forEach((n) => {
        n.date_filter = false;
    ...
    self.nodes.forEach((n) => {
      var did_match = _.some(conditions[0], (regexp) => (
        regexp.test(n.id) ||
  ...
  self.update = function (soft, friction) {
    ...
    if (!soft) {
      ...
      network_layout.nodes(self.draw_me.all).links(self.draw_me.edges);
      update_network_string(self.draw_me.nodes.length, self.draw_me.edges.length);

      var edge_set = {};

      _.each(self.draw_me.edges, (d) => {
        d.pull = 0.0;
        var tag;
      ...
      link = self.network_svg
        .selectAll(".link")
        .data(self.draw_me.edges, (d) => d.id);
      ...
      rendered_nodes = self.network_svg
        .selectAll(".node")
        .data(self.draw_me.nodes, (d) => d.id);
      ...
      if (
        ...
        self.annotate_priority_clusters(helpers._networkCDCDateField, 36, 12);
      ...
      if (self._is_CDC_) {
        tables.draw_extended_node_table(self);
      } else {
    ...
    link.style("opacity", (d) => Math.max(node_opacity(d.target), node_opacity(d.source)));
    ...
    link
      ...
      .classed("selected_object", (d) => d.ref.length_filter && !self.hide_unselected);
    ...
    if (!soft) {
      ...
      network_layout.on("tick", () => {
        ...
        rendered_clusters.attr("transform", (d) => (
          "translate(" +
          (d.x = Math.max(
            d.rendered_size,
  ...
  function node_info_string(n) {
    ...
    if (!self._is_CDC_) {
      str =
        ...
        helpers.format_value(n.lcc, _defaultFloatFormat) +
        "</em>";
    } else {
  ...
  function render_binned_table(id, the_map, matrix) {
    ...
    if (matrix) {
      ...
      if (self.show_percent_in_pairwise_table) {
        var sum = _.map(matrix, (row) => _.reduce(
          row,
          (p, c) => p + c,
          0
        ));

        matrix = _.map(matrix, (row, row_index) => _.map(row, (c) => c / sum[row_index]));
      }
      ...
      var rows = the_table
        ...
        .selectAll("tr")
        .data(
          matrix.map((d, i) => [lookup[i]].concat(d))
        );
  ...
  function attribute_pairwise_distribution(id, dim, the_map, only_expanded) {
    var scan_from = only_expanded ? self.draw_me.edges : self.edges;
    var the_matrix = [];
  ...
  self._aux_populate_category_fields = function (d, k) {
    ...
    if (d["type"] === "String") {
      ...
      d["value_range"] = _.keys(
        _.countBy(graph_data.Nodes, (nd) => self.attribute_node_value_by_id(nd, k))
      );
    ...
    if (d["no-sort"]) {
      ...
      if (d.dimension > _maximumValuesInCategories) {
        const compressed_values = _.chain(self.nodes)
          .countBy((node) => self.attribute_node_value_by_id(
            node,
            d["raw_attribute_key"]
          ))
          .pairs()
  ...
  function attribute_cluster_distribution(the_cluster, attribute_id) {
    if (attribute_id && the_cluster) {
      return the_cluster.children.map((d) => self.attribute_node_value_by_id(d, attribute_id));
    }
    return null;
  }
  ...
  function cluster_info_string(id) {
    ...
    if (self._is_CDC_) {
      ...
      str =
        ...
        helpers.format_value(the_cluster.cc, _defaultFloatFormat) +
        "</em>";
    }
  ...
  self.select_some_clusters = function (condition) {
    return self.clusters.filter((c, i) => _.some(c.children, (n) => condition(n)));
  };
  ...
  self._distance_gate_options = function (threshold) {
    ...
    return {
      "edge-styler": function (element, d, network) {
        var e_type = helpers.get_edge_type(
          d,
          network.edge_types,
        ...
        if (e_type !== "") {
          d3.select(element).style(
            ...
            network._edge_colorizer(
              helpers.get_edge_type(d, network.edge_types, network.edge_cluster_threshold)
            )
  ...
  self._social_view_options = function (
    ...
    return {
      "edge-styler": function (element, d, network) {
        var e_type = helpers.get_edge_type(d);
        if (e_type !== "") {
          d3.select(element).style(
            "stroke",
            network._edge_colorizer(helpers.get_edge_type(d))
          ); //.style ("stroke-dasharray", network._edge_dasher (d["edge_type"]));
  ...
  self.load_nodes_edges = function (
    ...
    if (edges_and_attributes && edges_and_attributes.length) {
      ...
      if (self._is_CDC_) {
        tables.draw_extended_node_table(self, self.json.Nodes);
      } else {
        tables.draw_node_table(self, self.extra_node_table_columns, self.json.Nodes);
      }
      ...
      */
      ...
      var _injected_column_subcluster_button_handler = function (
        ...
        //cv.annotate_priority_clusters(helpers._networkCDCDateField, 36, 12);
        //cv.handle_attribute_categorical("recent_rapid");
        cv._refresh_subcluster_view(self.today || helpers.getCurrentDate());
      };
      ...
      var injected_column = [
        {
          ...
          generator: function (cluster) {
            return {
              ...
              callback: function (self, element, payload) {
                var this_cell = d3.select(element);
                this_cell.text(Number(payload[0]) + " " + annotation + " nodes. ");
                var other_clusters = [];
                if (payload[1]) {
  ...
  var network_layout = d3.layout
    ...
    .linkDistance((d) =>
      link_scale(d.length) * l_scale * 0.2 //Math.max(d.length, 0.005) * l_scale * 10;
    )
    .linkStrength((d) => {
      if (self.showing_on_map) {
        return 0.01;
  ...
  if (options) {
    ...
    if (_.isArray(options["expand"])) {
      self.expand_some_clusters(
        _.filter(self.clusters, (c) => options["expand"].indexOf(c.cluster_id) >= 0)
      );
    }
